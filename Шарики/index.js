...
/*Рандомим скорость ветра*/
function getRandomInt(min, max) {
    return Math.floor(Math.random() * (max - min)) + min;
}
let wind = getRandomInt(0, 20);
...
setInterval(() => {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        k++;
        /*if (oldCoordsY[i] > -40) {
            oldCoordsX[i] = oldCoordsX[i] - 10;
            airBaloon.style.left = oldCoordsX[i] + 'px';
            airBaloon.style.transition = '2s';
        }*/
    }
}, 500)
...
let func = (airBaloon, i) => {
    ...
    if (oldCoordsY[i] > -40) {
        ...
        oldCoordsX[i] = oldCoordsX[i] + 0.01;
        airBaloon.style.right = oldCoordsX[i] + 'px';
    } else {
        airBaloon.remove();
    }
